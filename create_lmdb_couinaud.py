...
class LMDB_DatasetProcessor:
    def __init__(self, type, train_or_val, input_dir, input_dir_vessel, output_dir, dir_name, mask_dir_name, npz_keys,
                 spacing, num_phase,
                 liver_bound, logger=None):
        self.input_dir = input_dir
        self.output_dir = output_dir
        self.input_dir_vessel = input_dir_vessel
    ...
    def process(self, input_dir, mask_dir, mask_vessel_dir, name, args, type, save_dir):
        try:
            ...
            if ext == '.gz':
                ...
                # init_mask_vessel = sitk.ReadImage(os.path.join(mask_vessel_dir, name))
                ...
                #     init_mask_vessel = self.interpolate(init_mask_vessel, self.spacing)
                #
                # ct = sitk.GetArrayFromImage(init_ct)
                # mask = sitk.GetArrayFromImage(init_mask)
                # mask_vessel = sitk.GetArrayFromImage(init_mask_vessel)
                # ct = np.expand_dims(ct, axis=0)
                # mask = np.expand_dims(mask, axis=0)
                # mask_vessel = np.expand_dims(mask_vessel, axis=0)
                # c, d, h, w = mask.shape
                ...
                mask_vessel = sitk.GetArrayFromImage(init_mask_vessel)  # [D, H, W]
                ct = np.expand_dims(ct, axis=0)
                mask = np.expand_dims(mask, axis=0)
                mask_vessel = np.expand_dims(mask_vessel, axis=0)
                ...
                if self.spacing != 0:
                    ...
                    if type == 'train':
                        data_dict = {
                            ...
                            "label_vessel": MetaTensor(mask_vessel, affine=affine_matrix),
                        }
                        ...
                        mask_vessel = np.array(data_dict["label_vessel"])
                    else:
                        data_dict = {
                            "image": MetaTensor(ct, affine=affine_matrix),
                            "label_vessel": MetaTensor(mask_vessel, affine=affine_matrix),
            ...
            if type == 'train':
                for idx in range(d):
                    ...
                    mask_vessel_i = torch.from_numpy(mask_vessel_i.astype(np.uint8))
                    data = torch.stack([high_bytes, low_bytes, mask_i, mask_vessel_i], 0)
                ...
                return name_save, ct_min, ct.shape, affine_matrix, [c_init, w_init, h_init, d_init, spacing[0]], slice_dict
            else:
                for idx in range(d):
                    ct_i = ct[:, idx]
                    high_bytes, low_bytes = self.encode_ct_data(ct_i)
                ...
                for idx in range(d):
                    mask_vessel_i = mask_vessel[:, idx]
                    mask_vessel_i = torch.from_numpy(mask_vessel_i.astype(np.uint8))
                    data3 = torchvision.io.encode_png(mask_vessel_i.reshape(1, -1, w))
                    bin_data3 = pickle.dumps(data3)
                    slice_dict[str(name_save + "_label_vessel" + f"_{idx}").encode()] = bin_data3
    ...
    def read_file_list(self):
        ...
        args_list_total = []
        for input_dir, input_dir_vessel in zip(self.input_dir, self.input_dir_vessel):

            print("The root path of datasets is ", input_dir, input_dir_vessel)

            image_dir = self.dir_name[0]
            mask_dir = self.mask_dir_name[0]
            vessel_dir = self.mask_dir_name[0]

            if self.type == 'npz':
                # 暂时不用 如果是npz需要使用 到时候再改
                tmp_args_list = [(os.path.join(input_dir, image_dir), 0, i, self) for i in
                                 os.listdir(os.path.join(input_dir, image_dir))]
            elif self.type == 'nii.gz':
                tmp_args_list = [
                    (os.path.join(input_dir, image_dir), os.path.join(input_dir, mask_dir),
                     os.path.join(input_dir_vessel, vessel_dir), i, self) for i
                    in
                    os.listdir(os.path.join(input_dir, image_dir))]
    ...
    def save_lmdb(self, args_list, type, save_dir):
        ...
        new_args_list = [(item[0], item[1], item[2], item[3], item[4], type, save_dir) for item in args_list]
        pool = Pool(20)
        ...
        with env.begin(write=True) as txn:
            for name, ct_min, shape, affine_matrix, result_list, slice_dict in ret:
                print(name, ct_min, shape, affine_matrix, result_list)
                keys_tmp.append((name, ct_min, shape, affine_matrix, result_list))
    ...
    def running_npz_5_splits(self, n_splits=5, seed=42):
        from sklearn.model_selection import KFold
        args_list_total = self.read_file_list()

        new_args_list_total = []
        for item in args_list_total:
            file_name = item[3]
            dirs = [item[0], item[1], item[2]]
            ret = check_file_in_dirs(file_name, dirs)
            if ret:
                new_args_list_total.append(item)

        args_list_total = new_args_list_total
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(args_list_total)):
            train_list = [args_list_total[i] for i in train_idx]
            val_list = [args_list_total[i] for i in val_idx]

            print(f"Fold {fold + 1}/{n_splits}")
            print(f"  Train num: {len(train_list)}")
            print(f"  Val num:   {len(val_list)}")

            # 保存 LMDB
            fold_dir = os.path.join(self.output_dir, f'fold{fold + 1}')
            os.makedirs(fold_dir, exist_ok=True)

            self.save_lmdb(train_list, 'train', os.path.join(fold_dir, 'train'))
            self.save_lmdb(val_list, 'val', os.path.join(fold_dir, 'val'))
...
def create_lmdb_entry_point():
    ...
    #                     default=['/data0/scj/datasets/肝八段数据集/Couinaud/PuJian_20241109/',
    #                              '/data0/scj/datasets/肝八段数据集/Couinaud/abdomen_3d_reconstruction/'])
    # parser.add_argument('--input_dir_vessel', type=str, nargs='+',
    #                     default=['/data0/scj/datasets/肝八段数据集/Vessel/PuJian_20241109/',
    #                              '/data0/scj/datasets/肝八段数据集/Vessel/abdomen_3d_reconstruction/'])

    parser.add_argument('--input_dir', type=str, nargs='+',
                        default=['/data0/scj/datasets/肝八段数据集/Couinaud/Task08_HepaticVessel/'])
    parser.add_argument('--input_dir_vessel', type=str, nargs='+',
                        default=['/data0/scj/datasets/肝八段数据集/Vessel/Task08_HepaticVessel/'])
    ...
    parser.add_argument('--output_dir', type=str,
                        default="/data0/scj/datasets/肝八段数据集/Couinaud_lmdb/Couinaud_public")
    ...
    processor = LMDB_DatasetProcessor(
        ...
        input_dir_vessel=args.input_dir_vessel,
    ...
    processor.running_npz_5_splits()

def check_file_in_dirs(file_name, dir_list):
    missing_dirs = []
...
def read_lmdb_test():
    ...
    def save_niigz(img, mask, mask_vessel, save_dir):
        # 保存路径
        os.makedirs(save_dir, exist_ok=True)
        ct_save_path = os.path.join(save_dir, f"{case_id}_ct.nii.gz")
        mask_save_path = os.path.join(save_dir, f"{case_id}_mask.nii.gz")
        mask_vessel_save_path = os.path.join(save_dir, f"{case_id}_vessel_mask.nii.gz")
        spacing = (1.5, 1.5, 2.0)
        ...
        mask_vessel_array = mask_vessel[0].cpu().numpy()  # shape: [D, H, W]
        mask_vessel = sitk.GetImageFromArray(mask_vessel_array)
        mask_vessel.SetSpacing((spacing[2], spacing[1], spacing[0]))  # 注意：SimpleITK 是 (x, y, z)
        sitk.WriteImage(mask_vessel, mask_vessel_save_path)

    input_dir = "/data0/scj/datasets/肝八段数据集/Couinaud_lmdb/Couinaud_private/fold1/train"
    ...
    new_list = []
    num_multiphase = 1
    for ct_name, value, (_, max_depth, width, height),aff, result_list in info:
        info_name[ct_name] = (value, num_multiphase, (0, max_depth, width, height), aff,result_list)
        new_list.append(ct_name)
    print(info_name)
    print(new_list)
    case_id = 'PuJian024_0002'


    #  读的是训练集
    ct_min, num_multiphase, i, aff, shape = info_name[case_id]
    print(ct_min, num_multiphase, i, aff, shape, "111111111111111")
    env = lmdb.open(input_dir, readonly=True, lock=False,
                    readahead=False, meminit=False, create=False)
    ...
    print(mask_vessel.shape)  # c,d,h,w
    save_dir = input_dir
    save_niigz(img, mask, mask_vessel, save_dir)

    out_phase_id = 0
    image = img.permute(0, 3, 2, 1).contiguous()[0]
    mask = mask.permute(0, 3, 2, 1).contiguous()[0]
    mask_vessel = mask_vessel.permute(0, 3, 2, 1).contiguous()[0]
    print(image.shape)  # w h d
    print(mask.shape)  # w h d
    print(mask_vessel.shape)  # w h d

    # # 读验证集或者测试集
    # ct_min, num_multiphase, i, affine_matrix, shape = info_name[case_id]
    # print(ct_min, num_multiphase, i, affine_matrix, shape, "111111111111111")
    # env = lmdb.open(input_dir, readonly=True, lock=False,
    #                 readahead=False, meminit=False, create=False)
    # txn = env.begin(buffers=True)
    # img_all = []
    ...
    # mask_vessel_all = []
    # for t in range(i[0], i[1]):
    #     bin_data = txn.get(str(case_id + "_image" + f"_{t}").encode())
    #     data = torchvision.io.decode_png(pandas.read_pickle(io.BytesIO(bin_data)))
    ...
    #     bin_data = txn.get(str(case_id + "_label" + f"_{t}").encode())
    #     data = torchvision.io.decode_png(pandas.read_pickle(io.BytesIO(bin_data)))
    ...
    #
    # for t in range(i[0], i[1]):
    #     bin_data = txn.get(str(case_id + "_label_vessel" + f"_{t}").encode())
    #     data = torchvision.io.decode_png(pandas.read_pickle(io.BytesIO(bin_data)))
    #     mask_vessel = torch.reshape(data, (-1, i[-2], i[-1]))[:num_multiphase, :, :]
    #     mask_vessel_all.append(mask_vessel)
    # mask_vessel = torch.stack(mask_vessel_all, 1)
    #
    # print(img.shape)  # c,d,h,w
    # print(mask.shape)  # c,d,h,w
    # print(mask_vessel.shape)  # c,d,h,w
    # save_dir = input_dir
    # save_niigz(img, mask, mask_vessel, save_dir)
...
if __name__ == '__main__':
    ...
    input_dir = "/data0/scj/datasets/肝八段数据集/Couinaud_lmdb/Couinaud_private/fold1/val"
    env = lmdb.open(input_dir, readonly=True, lock=False,
                    readahead=False, meminit=False, create=False)
    filename_list = []
    info = []
