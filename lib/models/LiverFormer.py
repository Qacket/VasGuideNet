...
from lib.models.liverformer_package.neural_network import SegmentationNetwork
from lib.models.liverformer_package.vit_modeling import Transformer
from lib.models.liverformer_package.vit_modeling import CONFIGS as CONFIGS_ViT
    ...
    def __init__(
        ...
        if self.is_max:
            ...
            cfg = {
                ...
                "num_queries": (
                    num_classes if num_queries is None else num_queries
                ),  # N=K if 'fixed matching', else default=100,
                "nheads": 8,
            ...
            from lib.models.liverformer_package.mask2former_transformer_decoder3d import (
...
class LiverFormer(nn.Module):
    """
    3D LiverFormer wrapper that matches the training pipeline:
    - input: [B, C, D, H, W]
    - output: logits [B, out_channels, D, H, W]
    - no activation inside; loss will handle softmax
    """
    def __init__(
        self,
        in_channels: int,
        ...
        super().__init__()
        self.backbone = Generic_TransUNet_max_ppbp(
            ...
            # deep supervision on to produce final seg head; we will pick the last one
            deep_supervision=True,
            # return raw logits (loss will apply softmax)
            final_nonlin=lambda x: x,
            # use standard U-Net up/down path (no mask2former-style decoder)
            is_max=False,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        y = self.backbone(x)
        # deep_supervision=True returns a tuple: (final, aux..., ...)
