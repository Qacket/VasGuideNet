...
class PatchExpand3D(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=2, norm_layer=nn.LayerNorm):
        ...
        #     else nn.Identity()
        # )
        self.conv = nn.Conv3d(dim * 2, dim // 2, 3, 1, 1)
        self.norm = norm_layer(dim // 2)

    def forward(self, x, D_vessel=None):
        """
        x: B, D*H*W, C
        """
        ...
        x = x.permute(0, 2, 1).view(B, C, W, H, D)
        F_up = F.interpolate(x, scale_factor=2, mode="trilinear", align_corners=False)
        vessel_up = F.interpolate(
            D_vessel, scale_factor=2, mode="trilinear", align_corners=False
        )
        out = torch.cat([vessel_up, F_up], dim=1)
        out = self.conv(out).permute(0, 2, 3, 4, 1).view(B, -1, C // 2)
        out = self.norm(out)
        return out
...
class FinalPatchExpand_X4_3D(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=4, norm_layer=nn.LayerNorm):
        ...
        self.output_dim = dim
        self.conv = nn.Conv3d(dim * 2, self.output_dim, 3, 1, 1)
        self.norm = norm_layer(self.output_dim)

    def forward(self, x, D_vessel):
        """
        x: B, D*H*W, C
        """
        W, H, D = self.input_resolution
        B, L, C = x.shape
        x = x.permute(0, 2, 1).view(B, C, W, H, D)
        F_up = F.interpolate(x, scale_factor=4, mode="trilinear", align_corners=False)
        vessel_up = F.interpolate(
            D_vessel, scale_factor=4, mode="trilinear", align_corners=False
        )
        out = torch.cat([vessel_up, F_up], dim=1)
        out = self.conv(out).permute(0, 2, 3, 4, 1).view(B, -1, self.output_dim)
        out = self.norm(out)
        return out
...
class MyDecoderLayer3D(nn.Module):
    ...
    def forward(self, x1, x2, vessel_mask=None):
        ...
        if self.last_layer:
            out = self.last_layer(
                self.layer_up(tran_layer_2, vessel_mask)
                .view(b, 4 * w, 4 * h, 4 * d, -1)
                .permute(0, 4, 1, 2, 3)
            )
        else:
            out = self.layer_up(tran_layer_2, vessel_mask)
        return out


class MISSFormer3D_tokenDecoder(nn.Module):
    def __init__(
        self,
        input_dims=1,
        num_classes=9,
        ...
        self.makeTokens = ConvStream(in_chans=1, out_chans=[64, 128, 320, 512])

    def forward(self, x):
        x, vessel_mask = x[:, 0].unsqueeze(1), x[:, 1].unsqueeze(1)
        x = x.permute(0, 1, 4, 2, 3)
        vessel_mask = vessel_mask.permute(0, 1, 4, 2, 3)
        vessel_mask_tokens = self.makeTokens(vessel_mask)  # [512,320,128,64],
        ...
        tmp_3 = self.decoder_3(
            ...
            vessel_mask_tokens[3],
        )
        tmp_2 = self.decoder_2(
            tmp_3, bridge[2].permute(0, 2, 3, 4, 1), vessel_mask_tokens[2]
        )
        tmp_1 = self.decoder_1(
            tmp_2, bridge[1].permute(0, 2, 3, 4, 1), vessel_mask_tokens[1]
        )
        tmp_0 = self.decoder_0(
            tmp_1, bridge[0].permute(0, 2, 3, 4, 1), vessel_mask_tokens[0]
        )
...
class Basic_Conv3x3(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.conv = nn.Conv3d(in_chans, out_chans, 3, stride, padding, bias=False)
...
class ConvStream(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(self.conv_chans) - 1):
            ...
            self.convs.append(Basic_Conv3x3(in_chan_, out_chan_, stride=strides[i]))
